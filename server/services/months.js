...
// computed server-side from date in a configured timezone").
...
/**
 * Ensures a `months` row exists for `monthKey`, creating it only if
 * missing. MUST be called with the same transaction as the write that
 * needs it — never from a GET path (design.md: "reads never create it").
 *
 * @param {import('knex').Knex.Transaction} trx
 * @param {string} monthKey
 * @returns {Promise<object>} the (existing or newly created) months row
 */
export async function materializeMonth(trx, monthKey) {
  await trx('months').insert({ month_key: monthKey }).onConflict('month_key').ignore();
  return trx('months').where({ month_key: monthKey }).first();
}
 ...
 */
export async function getMonthPayload(monthKey, executor = db) {
  const monthRow = await executor('months').where({ month_key: monthKey }).first();
  const materialized = Boolean(monthRow);

  const budgetRows = await executor('category_budgets').where({ month_key: monthKey });
  const budgets = {};
  ...
  // "Postgres Schema (DDL)" section, so no balance is ever persisted.
  const { rows: categoryRows } = await executor.raw(
    `SELECT c.id,
     ...
     FROM categories c
     LEFT JOIN category_budgets b ON b.category_id = c.id AND b.month_key = ?
